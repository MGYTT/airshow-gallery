...
async function getPublishedShows() {
  ...
  return shows.filter((show) => {
    ...
    if (!isValidId(id) || seen.has(id)) return false;
    seen.add(id);
    return true;
  });
}
...
async function getPublishedEvents() {
  ...
  return events.filter((event) => {
    ...
    if (!isValidSlug(slug) || seen.has(slug)) return false;
    seen.add(slug);
    return true;
  });
}
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const show of shows) {
    sitemapEntries.push({
      ...
      lastModified:
        toValidDate(show.updated_at) ?? toValidDate(show.created_at),
      changeFrequency: "monthly",
      priority: 0.8,
    });
  }
  ...
  for (const event of events) {
    sitemapEntries.push({
      ...
      lastModified:
        toValidDate(event.updated_at) ?? toValidDate(event.published_at),
      changeFrequency: "weekly",
      priority: 0.9,
    });
  }
