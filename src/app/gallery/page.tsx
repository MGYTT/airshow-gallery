  ...
  return (
    <>
      <style>{`
        ...
        .lb-counter { position:fixed; top:var(--space-5); left:50%; transform:translateX(-50%); background:rgba(0,0,0,0.55); backdrop-filter:blur(8px); border:1px solid rgba(255,255,255,0.1); color:rgba(255,255,255,0.8); font-size:var(--text-xs); font-weight:600; padding:var(--space-2) var(--space-4); border-radius:var(--radius-full); }
      ...
      <div className="lb" onClick={onClose}>
        ...
        <div className="lb-bar" onClick={e => e.stopPropagation()}>
          ...
          {show && (
            <Link href={`/pokaz/${show.id}`} style={{ display:"inline-flex", alignItems:"center", gap:"var(--space-2)", fontSize:"var(--text-xs)", fontWeight:600, color:"rgba(255,255,255,0.8)", background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.15)", padding:"var(--space-2) var(--space-3)", borderRadius:"var(--radius-md)", textDecoration:"none", flexShrink:0, backdropFilter:"blur(6px)" }}>
...
function PhotoMasonry({ list, onOpen }: { list: Photo[]; onOpen: (i: number) => void }) {
  if (list.length === 0) return (
    <div style={{ minHeight:"40dvh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"var(--space-4)", color:"var(--color-text-faint)", paddingBottom:"var(--space-16)" }}>
      ...
      <p style={{ fontSize:"var(--text-sm)", fontWeight:600 }}>Brak zdjęć dla wybranego pokazu</p>
    </div>
  );
...
function Pill({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ display:"inline-flex", alignItems:"center", padding:"var(--space-2) var(--space-4)", borderRadius:"var(--radius-full)", fontSize:"var(--text-xs)", fontWeight:600, cursor:"pointer", border:"1.5px solid", whiteSpace:"nowrap", transition:"all var(--transition)", background:active?"var(--color-accent)":"var(--color-surface)", color:active?"#fff":"var(--color-text-muted)", borderColor:active?"var(--color-accent)":"var(--color-border)" }}>
      {children}
    </button>
  );
}
...
function GalleryContent() {
  ...
  // ✅ KLUCZOWA ZMIANA: ładujemy z Supabase przez API
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
  ...
  const filteredList = useMemo<Photo[]>(() => {
    if (activeShow === "all") return photos;
    return photos.filter(p => p.showId === activeShow);
  }, [activeShow, photos]);
  ...
  return (
    <>
      <style>{`
        ...
        .g-filterbar { position:sticky; top:63px; z-index:50; background:var(--color-bg); border-bottom:1px solid var(--color-divider); padding:var(--space-3) 0; }
        .g-pills { display:flex; gap:var(--space-2); flex-wrap:wrap; flex:1; min-width:0; }
      ...
      <div className="g-header">
        ...
        <div className="container" style={{ position:"relative", zIndex:1 }}>
          <span className="badge" style={{ marginBottom:"var(--space-4)" }}><Filter size={11}/> Galeria</span>
          ...
          <div style={{ display:"flex", gap:"var(--space-5)", flexWrap:"wrap" }}>
            <span className="g-meta-chip"><Images size={13}/><strong style={{ color:"var(--color-text)", fontVariantNumeric:"tabular-nums" }}>{total}</strong> zdjęć</span>
            {activeShow === "all"
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:"var(--space-4)", flexWrap:"wrap" }}>
            ...
            <div className="g-view-toggle">
              {(["grid","shows"] as const).map(v => (
                <button key={v} className="g-view-btn" onClick={() => setView(v)} aria-pressed={view===v} style={{ background:view===v?"var(--color-surface-offset)":"transparent", color:view===v?"var(--color-text)":"var(--color-text-faint)" }}>
      ...
      {lbIndex !== null && total > 0 && (
        <Lightbox list={filteredList} index={lbIndex} onClose={closeLb} onPrev={prevLb} onNext={nextLb} shows={shows}/>
      )}
    </>
  );
}
