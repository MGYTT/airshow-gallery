...
export default function StoriesArchive() {
  ...
  async function load() {
    ...
    if (res.ok) {
      const data: MappedStory[] = await res.json();
      setStories(data);
      setFiltered(data);

      // Unikalne pokazy z relacji
      const map = new Map<string, ShowFilter>();
      data.forEach(s => {
        if (!map.has(s.showId)) {
          // showId to string — tytuł nie jest dostępny, pobieramy z listy shows
          map.set(s.showId, { id: s.showId, name: s.showId, year: 0 });
        }
      });

      // Pobierz nazwy pokazów
      ...
      if (sr.ok) {
        ...
        // Filtruj tylko te, które mają relacje
        const withStories = showData.filter(s => map.has(s.id));
        setShows(withStories);
      }
    }
    setLoading(false);
  }
  ...
  function openStory(globalIdx: number) {
    // globalIdx odnosi się do filtered — znajdź w stories
  ...
  return (
    <>
      <style>{`
        ...
        .sa-filter-btn{display:inline-flex;align-items:center;gap:var(--space-2);padding:var(--space-2) var(--space-4);border-radius:var(--radius-full);border:1px solid var(--color-border);background:var(--color-surface);font-size:var(--text-sm);color:var(--color-text-muted);cursor:pointer;transition:all .15s;white-space:nowrap}
        .sa-filter-btn:hover,.sa-filter-btn.active{border-color:var(--color-primary);color:var(--color-primary);background:oklch(from var(--color-primary) l c h / .06)}
        ...
        <div className="sa-hero">
          ...
          <div className="sa-hero-content">
            ...
            <h1 style={{ fontFamily:"var(--font-display)", fontWeight:900, fontSize:"var(--text-2xl)", letterSpacing:"-0.04em", lineHeight:1.05, marginBottom:"var(--space-4)" }}>
              ...
              <span style={{ color:"var(--color-accent)" }}>od środka</span>
            </h1>
          ...
          {!loading && (
            <div className="sa-stats">
              {[
                ...
                { label:"Klatek",    value: stories.reduce((s, r) => s + r.frames.length, 0) },
              ].map(({ label, value }) => (
          ...
          <div className="sa-grid">
            {loading ? (
              ...
              <div className="sa-empty">
                ...
                {(query || showId !== "all") && (
                  <button
                    onClick={() => { setQuery(""); setShowId("all"); }}
              ...
              filtered.map((story, i) => (
                ...
                >
                  ...
                  <div className="sa-card-frames-badge">
                    <Images size={10}/> {story.frames.length}
                  </div>
                  ...
                  <div className="sa-card-body">
                    ...
                    {/* Podtytuł */}
                    {story.subtitle && (
                      <p style={{ fontSize:11, color:"rgba(255,255,255,.55)", lineHeight:1.3 }}>
                        {story.subtitle}
                      </p>
                    )}
